---
layout: tony
permalink: /chinese/
title: 蔡天文
kicker: 中文版本
description: 宾夕法尼亚大学沃顿商学院 Daniel H. Silberberg 讲席教授、统计与数据科学教授
header_image: /assets/img/TonyCai22.jpg
header_image_alt: 蔡天文教授肖像
lang: zh-CN
nav: true
nav_order: 8
---

<!-- <div class="tc-callout">
  <p><strong>蔡天文</strong>，宾夕法尼亚大学沃顿商学院 Daniel H. Silberberg 讲席教授、统计与数据科学教授。</p>
</div> -->

<div class="tc-chinese-overview">
<p>
蔡天文是宾夕法尼亚大学沃顿商学院Daniel H. Silberberg讲席教授, 统计与数据科学教授。他还是宾大应用数学与计算科学研究生项目的核心成员，并在佩雷尔曼医学院生物统计学、流行病学与信息学系担任学术职务。他于1996年在康奈尔大学获得统计学博士学位。
</p>

<p>  
蔡教授的研究为现代数据科学与人工智能奠定统计学基础，当前主要关注迁移学习、差分隐私、联邦学习与分布式学习、高维统计、因果推断及统计决策论等方向。其研究的核心主题是在异质、敏感与分布式数据场景中实现可靠的统计推断：来自相关总体的信息如何助力目标分析，隐私与通信约束如何影响统计精度，以及如何在避免负迁移的前提下将方法迁移至新场景。这些问题广泛存在于当代科技应用中——数据往往分布在不同的机构、总体、研究项目与设备之间，而稳健的结论必须在尊重隐私并适应异质性的前提下得出。
</p>

<p> 
蔡教授的最新工作为隐私保护估计与检验、联邦学习、迁移学习及个性化治疗决策建立了决策论框架。这些议题在人工智能领域中日益重要，因为现代系统必须从大规模、分布式、异质的数据源中持续学习，同时兼顾隐私、可靠性与资源约束。他的研究通过阐明学习何时可行、信息需求的理论下界，以及在约束条件下如何设计最优算法，为可信赖人工智能奠定了坚实的统计学基础。相关方法可应用于生物医学研究、公共卫生、基因组学、金融学、分布式学习系统以及大规模科学协作等多个领域。
</p>

<p> 
蔡教授的学术贡献覆盖现代统计学的多个重要方向。他发展了一系列具有影响力的理论与方法，包括高维协方差与精度矩阵估计、稀疏主成分分析、图模型、回归与高维检验，以及非参数估计、自适应方法与不确定性量化。在大规模多重检验方面，他的研究有效解决了复杂高维场景下的功效与错误发现率控制问题；而对二项置信区间、奇异子空间扰动理论及t-SNE的理论分析，则提供了广泛使用的工具与基准方法。这些成果中，许多已成为现代人工智能与机器学习的重要支撑，尤其涉及高维结构处理、谱方法、降维、不确定性量化及基于复杂数据的可靠推断等核心环节。
</p>

<p> 
蔡教授的研究在应用科学领域也产生了深远的实际影响。例如，Brown–Cai–DasGupta关于二项置信区间的论文已被引用超过4800次，广泛应用于医学、公共卫生、临床试验、流行病学、质量控制、遗传学及相关领域。在基因组学方面，其工作涵盖大规模多重检验、差异共表达分析与基因网络推断，在这些领域中，严谨的统计方法对可靠的科学发现尤为关键。蔡教授的研究始终将深刻的理论洞察与实践导向的方法相结合，发展出统计最优且具备自适应能力的算法，同时精准刻画了在高维性、结构复杂性、隐私、通信与异质性等多重约束下的理论极限。总体而言，他的研究深刻影响了统计理论、机器学习、生物医学、基因组学及数据驱动的科学发现等多个领域。
</p>

<p> 
蔡教授曾荣获多项重要学术荣誉，包括国际数理统计学会Wald纪念奖与讲座、COPSS总统奖、美国统计学会Noether杰出学者奖、美国统计学会Leo Breiman奖、国际数理统计学会奖章讲座、剑桥大学Peter Whittle讲座、Bernoulli学会Laplace讲座，以及泛华统计学会杰出成就奖。他是国际数理统计学会（IMS）与美国科学促进会（AAAS）会士，曾任国际数理统计学会主席及泛华统计学会主席。在学术编辑方面亦贡献卓著，曾担任《统计年刊》（The Annals of Statistics）联合主编，并长期担任《统计年刊》、《美国统计学会会刊》（JASA）和《皇家统计学会会刊B辑》（JRSS-B）等顶级期刊副主编。
</p>
</div>

<div class="tc-section-divider" aria-hidden="true"></div>

<div class="tc-fact-grid">
  <section class="tc-fact">
    <h2>教育背景</h2>
    <p>康奈尔大学统计学博士，1996。</p>
  </section>
  <section class="tc-fact">
    <h2>学术职位</h2>
    <ul>
      <li>沃顿商学院 Daniel H. Silberberg 讲席教授</li>
      <li>沃顿商学院统计与数据科学教授</li>
      <li>宾大应用数学及计算科学教授</li>
      <li>宾大医学院生物统计、流行病学及信息学系资深学者</li>
    </ul>
  </section>
  <section class="tc-fact">
    <h2>行政职位</h2>
    <p>沃顿商学院副院长，2017–2020。</p>
  </section>
  <section class="tc-fact">
    <h2>研究方向</h2>
    <ul>
      <li>统计机器学习与高维统计</li>
      <li>大规模统计推断与统计决策论</li>
      <li>非参数函数估计与函数数据分析</li>
      <li>金融工程与基因组学中的数据分析</li>
    </ul>
  </section>
</div>

<div class="tc-section-divider" aria-hidden="true"></div>

<h2 id="chinese-special-lecture-heading">国际数理统计学会主席致辞</h2>
<ol class="tc-lecture-list" aria-labelledby="chinese-special-lecture-heading">
  <li class="tc-lecture-card" data-disclosure-item>
    <p class="tc-lecture-meta">联合统计会议（JSM 2025）</p>
    <div class="tc-paper-title-row tc-lecture-title-row">
      <h2>
        <button
          class="tc-paper-title-toggle"
          id="chinese-ims-presidential-address-2025-toggle"
          type="button"
          aria-expanded="false"
          aria-controls="chinese-ims-presidential-address-2025-details"
          data-paper-toggle
        >
          处在十字路口的统计学：人工智能时代的挑战与机遇
        </button>
      </h2>
      <a
        class="tc-paper-pdf-pill"
        href="{{ '/assets/pdf/lectures/IMS_Presidential_Address_Chinese.pdf' | relative_url }}"
        target="_blank"
        rel="noopener noreferrer"
        aria-label="打开《处在十字路口的统计学：人工智能时代的挑战与机遇》PDF"
        >PDF</a
      >
    </div>
    <div
      class="tc-paper-abstract-panel tc-lecture-detail-panel"
      id="chinese-ims-presidential-address-2025-details"
      role="region"
      aria-labelledby="chinese-ims-presidential-address-2025-toggle"
      data-paper-panel
      hidden
    >
      <p class="tc-paper-abstract-label">摘要</p>
      <div class="tc-lecture-abstract-copy">
        <p class="tc-paper-abstract">
          当今世界正经历一场深刻的技术变革，其驱动力来自海量数据的爆炸式增长和以数据为驱动的人工智能的飞速发展。这场变革的核心正是统计学、机器学习与人工智能——这些强大的工具正在重塑我们的工作、生活和沟通方式。数据科学几乎在推动着每一个领域的创新。这一前所未有的转变，无论是在研究还是教育方面，都为统计学领域带来了巨大的机遇。随着对数据驱动洞察的需求不断增长，统计学家和数据科学家的角色变得愈发重要。在本次演讲中，我将探讨统计学在人工智能时代所面临的紧迫挑战与激动人心的机遇。
        </p>
      </div>
    </div>
  </li>
</ol>

<div class="tc-section-divider" aria-hidden="true"></div>

<h2 id="chinese-editorial-heading">学术期刊编委会</h2>
<div class="tc-table-wrap">
  <table class="tc-table" aria-labelledby="chinese-editorial-heading">
    <thead>
      <tr><th scope="col">职务</th><th scope="col">期刊</th><th scope="col">任期</th></tr>
    </thead>
    <tbody>
      <tr><td>主编</td><td>《统计年刊》（<em lang="en">The Annals of Statistics</em>）</td><td>2010–2012</td></tr>
      <tr><td>编委</td><td>《皇家统计学会会刊 B 辑》（<em lang="en">Journal of the Royal Statistical Society, Series B</em>）</td><td>2014–2018</td></tr>
      <tr><td>编委</td><td>《美国统计学会会刊》（<em lang="en">Journal of the American Statistical Association</em>）</td><td>2005–2010</td></tr>
      <tr><td>编委</td><td>《统计年刊》（<em lang="en">The Annals of Statistics</em>）</td><td>2004–2009</td></tr>
      <tr><td>编委</td><td>《泛华统计学会会刊》（<em lang="en">Statistica Sinica</em>）</td><td>2005–2011</td></tr>
      <tr><td>编委</td><td><em lang="en">Statistics Surveys</em></td><td>2006–2009</td></tr>
      <tr><td>编委会成员</td><td><em lang="en">Frontiers of Statistics</em> 丛书</td><td>2009–至今</td></tr>
      <tr><td>客座主编</td><td>《<em lang="en">Statistica Sinica</em>》多尺度方法特刊</td><td>—</td></tr>
      <tr><td>客座主编</td><td>《<em lang="en">Journal of Nonparametric Statistics</em>》首届 IMS-China 国际会议特刊</td><td>—</td></tr>
    </tbody>
  </table>
</div>

<div class="tc-section-divider" aria-hidden="true"></div>

<h2>主要荣誉</h2>
<ul class="tc-honors-list">
  <li><span class="tc-honor-year">2027</span><span>国际数理统计学会 (IMS) Wald 纪念奖与讲座</span></li>
  <li><span class="tc-honor-year">2026</span><span>美国统计学会 (ASA) Leo Breiman 奖</span></li>
  <li><span class="tc-honor-year">2026</span><span>世界华人数学家大会最佳论文金奖</span></li>
  <li><span class="tc-honor-year">2023–26</span><span>国际数理统计学会（IMS）候任主席（2023–2024）、主席（2024–2025）及前任主席（2025–2026）</span></li>
  <li><span class="tc-honor-year">2024</span><span>美国科学促进会会士（AAAS Fellow）</span></li>
  <li><span class="tc-honor-year">2023</span><span>美国统计学会 (ASA) Noether 杰出学者奖</span></li>
  <li><span class="tc-honor-year">2023</span><span>国际基础科学大会前沿科学奖</span></li>
  <li><span class="tc-honor-year">2021</span><span>伯努利学会 Laplace 讲座（第十届世界概率与统计大会）</span></li>
  <li><span class="tc-honor-year">2019</span><span>泛华统计学会 (ICSA) 杰出成就奖</span></li>
  <li><span class="tc-honor-year">2019</span><span>泛华统计学会（ICSA）杰出服务奖</span></li>
  <li><span class="tc-honor-year">2018</span><span>剑桥大学 Peter Whittle 讲座</span></li>
  <li><span class="tc-honor-year">2018</span><span>世界华人数学家大会最佳论文奖</span></li>
  <li><span class="tc-honor-year">2016–18</span><span>泛华统计学会（ICSA）候任主席、主席及前任主席</span></li>
  <li><span class="tc-honor-year">2012</span><span>柏林洪堡大学 Hermann Otto Hirschfeld 讲座</span></li>
  <li><span class="tc-honor-year">2010</span><span>第 28 届欧洲统计学大会论坛讲座</span></li>
  <li><span class="tc-honor-year">2009</span><span>国际数理统计学会（IMS）勋章讲座</span></li>
  <li>
    <span class="tc-honor-year">2008</span>
    <span><a href="http://en.wikipedia.org/wiki/COPSS_Presidents'_Award">世界统计学考普斯奖（COPSS Presidents’ Award）</a></span>
  </li>
  <li><span class="tc-honor-year">2006</span><span>国际数理统计学会会士（IMS Fellow）</span></li>
</ul>

<div class="tc-section-divider" aria-hidden="true"></div>

<h2>联系信息</h2>
<p>电话：215-898-8224 · 电邮：<a href="mailto:tcai@wharton.upenn.edu">tcai@wharton.upenn.edu</a></p>

<script defer src="{{ '/assets/js/paper-search.js' | relative_url }}"></script>

<section class="tc-visitor-counter" aria-label="Website visitor counter">
  <div class="statcounter">
    <script>
      var sc_project = 379152;
      var sc_invisible = 0;
      var sc_security = "";
    </script>
    <script src="https://secure.statcounter.com/counter/counter.js"></script>
    <noscript>
      <a href="https://statcounter.com/" target="_blank" rel="noopener noreferrer">
        <img
          class="statcounter"
          src="https://c.statcounter.com/379152/0//0/"
          alt="Website visitor counter"
          referrerpolicy="no-referrer-when-downgrade"
        >
      </a>
    </noscript>
  </div>
   <span>Visits</span>
</section>
